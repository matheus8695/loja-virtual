...
it('should create an order for the logged user when click in add product on cart and add the product in product_orders table', function () {
    $product = Product::factory()->create();

    Livewire::test(Cart\Create::class, ['product' => $product])
...
test('when a user already has an open order the system have to add the product to product_orders table', function () {
    ...
    Livewire::test(Cart\Create::class, ['product' => $product2])
        ->call('handleProductOrder');
...
it('should ensure the user only has one open order at a time', function () {
    ...
    Livewire::test(Cart\Create::class, ['product' => $product1])
        ->call('handleProductOrder', );
...
test('When the user tries to buy a product that is already in the order the system should just redirect to the dashboard route', function () {
    $product = Product::factory()->create();
